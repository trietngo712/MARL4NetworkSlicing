...
class NetworkEnvV14(ParallelEnv):
    ...
    def __init__(self,
        ...
        self.uniform_demand = uniform_demand
    ...
    def _get_obs(self, agent):
        ...
        elif self.uniform_demand is not None:
            new_task = Task(arrival_time=self.current_time, resource_demand={
                res_id: np.random.uniform(self.uniform_demand[agent]['low'], self.uniform_demand[agent]['high']) * self.resources[res_id].capacity
                for res_id in slice_obj.idx_to_resource
            })
        ...
        if new_task is not None:
            ...
            raise ValueError(f"No valid demand source provided for agent={agent} at time={self.current_time}. Please specify test_demand, traffic_path, or uniform_demand.")
    ...
    def step(self, actions):
        ...
        collective_time_spent_in_a_time_step = {agent: 0.0 for agent in self.agents}
        ...
        for agent, action in items:
            ...
            #print(action)
            #print(f'agent {agent} action {min(action)}')
            ...
            for idx, res_id in enumerate(slice_obj.idx_to_resource):
                ...
                if req > resource.available_capacity:
                    ...
                    #print(f'{agent} requested {normalized_action:.4f} for {res_id}, but only {resource.available_capacity:.4f} available. Adjusted to {req:.4f}')
        ...
        for agent in self.agents:
            ...
            for task in slice_obj.task_queue:
                if task.is_complete():
                    ...
                    collective_time_spent_in_a_time_step[agent] += frac
        ...
        e = {}
        e_a = {agent: {res_id: 0.0 for res_id in self.resources.keys() if 'mec' in res_id} for agent in self.agents}
        for res_id, resource in self.resources.items():
            energy = 0
            if 'mec' in res_id:
                ...
                for i  in range(len(order)):
                    ...
                    if start is None:
                        start = 0 
                    current_energy = resource.power_function(total_usage) * (active_times[o] - start)
                    energy += resource.power_function(total_usage) * (active_times[o] - start)
                    
                    for j in range(i, len(order)):
                        o_j = order[j]
                        coeff = partial_usage[o_j] / total_usage if total_usage > 0 else 0
                        
                        e_a[self.agents[o_j]][res_id] += current_energy * coeff
                ...
                if max(active_times) < 1:
                    ...
                    #for agent in self.agents:
                    #    e_a[agent][res_id] += resource.power_function(0) * (1 - max(active_times)) / len(self.agents)
        ...
        for i, agent in enumerate(self.agents):
            ...
            #        #agent_e.append((426.8324 - e[res_id]) / 200)
            #
            #        record_energy.append(e[res_id] * coeff)
            for res_id, resource in self.resources.items():
                if 'mec' in res_id:
                    agent_e.append((426.8324 - e_a[agent][res_id]) / 426.8324)
                    #agent_e.append((1000 - e_a[agent][res_id]) / 1000)

                    record_energy.append(e_a[agent][res_id])
        ...
        for agent in self.agents:
            recorder = self.recorders[agent]
            recorder.add_reward_component('penalty', 0)
            ...
            recorder.add_reward_component('stable', self.stable*queue_control * self.reward_scale)
        ...
        if overflow_detected:
            ...
            for agent in self.agents:
                ...
                self.recorders[agent].reward_components['penalty'][-1] = self.recorders[agent].reward_components['penalty'][-1] + self.penalty_reward
        ...
        if self.period_t % self.max_steps == 0:
            for agent in self.agents:
                recorder = self.recorders[agent]
                recorder.add_episode_end(self.current_time)            
    ...
    def _read_config(self, resource_path):
        if not resource_path or not os.path.exists(resource_path):
            ...
            config = [
                {"type": "mec", "id": 1, "resources": {"cpu": 4}},
                {"type": "link", "id": 1, "resources": {"bandwidth": 10}}
            ]
...
class Recorder():
    ...
    def add_reward_component(self, component_name, value):
        if component_name in self.reward_components:
            ...
            logger.warning(f"[Recorder.add_reward_component] Unknown component_name={component_name}, value={value}")
    ...
    def save_result(self, path):
        try:
            ...
            real_reward = {'real_reward': self.real_reward}
            ...
            data_to_save = {
                ...
                'real_reward.csv': real_reward,
                'reward_components.csv': self.reward_components
            }
...
class Resource():
    ...
    def power_function(self, utilization):
        ...
        #return 1000 * utilization * utilization  * utilization  # Cubic power function for MEC servers
...
class FairShareScheduler(BaseScheduler):
    def schedule(self, task_queue, res_id, allocated_amount, total_resource_allocation, server_power, current_time, delta = 1, recursive_depth=0):
        ...
        if (delta == 0 ) or (n_sub_tasks == 0):
            return delta
        
        share = allocated_amount / n_sub_tasks
        #print(f'share {share}')
        #if not np.isfinite(share) or share <= 1e-5:
        #    raise ValueError(
        #        f"Invalid share={share}, allocated={allocated_amount}, n_tasks={n_sub_tasks}, recursive_depth={recursive_depth}, delta={delta}, res_id={res_id}, current_time={current_time}"
        #    ) 
        ...
        for t in active_tasks_on_this_component:
            ...
            if (t.resource_demand[res_id] <= 1e-5) and (res_id not in t.completion_times):
                t.resource_demand[res_id] = 0
                t.completion_times[res_id] = current_time +  t_min_finish - t.arrival_time
                
        if (delta - t_min_finish) > 1e-5:
