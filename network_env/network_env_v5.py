...
class NetworkEnvV5(ParallelEnv):
        ...
        for agent in self.agents:
            ...
            for task in slice_obj.task_queue:
                if task.is_complete():
                    ...
                    reward_task = - (lambda_i * (end_to_end_latency / self.alpha)) - \
                                  (rho_i * (max(1e-5, task.accumulated_energy) / self.beta))
                    
                    reward_task = reward_task / 100
