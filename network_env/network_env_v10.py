...
class NetworkEnvV10(ParallelEnv):
    ...
    def step(self, actions):
        ...
        for agent in self.agents:
            ...
            x = np.array([(MAX_QUEUE_LENGTH - self.current_queue[agent][res_id] + (agent_requests[agent][res_id] / resource.capacity)) / (MAX_QUEUE_LENGTH ) for res_id, resource in self.resources.items()])
...
class Recorder():
    ...
    def add_conflict(self, current_time):
        self.conflic.append(current_time)
