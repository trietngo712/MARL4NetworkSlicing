...
class NetworkEnvV9(ParallelEnv):
    ...
    def step(self, actions):
        ...
        for agent in self.agents:
            ...
            systemic_energy[agent] = np.sum(list(resource_powers.values()))
            systemic_latency[agent] = np.sum([np.mean(list(t.resource_demand.values())) for t in slice_obj.task_queue]) + 1
        ...
        if overflow_detected:
            ...
            print(f'time {self.current_time}- reward {list(rewards.values())}')
